...
###########################################
mkdir ~/GPT0304_Backup
sgdisk --backup=~/GPT0304_Backup/All_Data_Partitions.gpt /dev/$dataDrive

for i in {2..5}; do sgdisk --delete=$i /dev/$dataDrive; done
sgdisk --sort /dev/$dataDrive
sgdisk --backup=~/GPT0304_Backup/G1data_Partition.gpt /dev/$dataDrive
sgdisk --load-backup=~/GPT0304_Backup/All_Data_Partitions.gpt /dev/$dataDrive

sgdisk --delete=1 /dev/$dataDrive
for i in {3..5}; do sgdisk --delete=$i /dev/$dataDrive; done
sgdisk --sort /dev/$dataDrive
sgdisk --backup=~/GPT0304_Backup/G2data_Partition.gpt /dev/$dataDrive
sgdisk --load-backup=~/GPT0304_Backup/All_Data_Partitions.gpt /dev/$dataDrive

for i in {1..2}; do sgdisk --delete=$i /dev/$dataDrive; done
for i in {4..5}; do sgdisk --delete=$i /dev/$dataDrive; done
sgdisk --sort /dev/$dataDrive
sgdisk --backup=~/GPT0304_Backup/G3data_Partition.gpt /dev/$dataDrive
sgdisk --load-backup=~/GPT0304_Backup/All_Data_Partitions.gpt /dev/$dataDrive

for i in {1..3}; do sgdisk --delete=$i /dev/$dataDrive; done
sgdisk --delete=5 /dev/$dataDrive
sgdisk --sort /dev/$dataDrive
sgdisk --backup=~/GPT0304_Backup/G4data_Partition.gpt /dev/$dataDrive
sgdisk --load-backup=~/GPT0304_Backup/All_Data_Partitions.gpt /dev/$dataDrive

for i in {1..4}; do sgdisk --delete=$i /dev/$dataDrive; done
sgdisk --sort /dev/$dataDrive
sgdisk --backup=~/GPT0304_Backup/G5data_Partition.gpt /dev/$dataDrive
sgdisk --load-backup=~/GPT0304_Backup/All_Data_Partitions.gpt /dev/$dataDrive

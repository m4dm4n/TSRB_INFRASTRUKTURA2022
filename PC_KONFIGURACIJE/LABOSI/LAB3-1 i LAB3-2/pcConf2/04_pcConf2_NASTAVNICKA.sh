...
saveDIR=~/BACKUP
mkdir -p $saveDIR/SSD/CONF2/NASTAVNICKA
mkdir -p $saveDIR/HDD/CONF2/NASTAVNICKA

echo "Spremam GPT sa svim particijama na diskovima"
sgdisk --backup=$saveDIR/SSD/CONF2/KABINETI/00_SSD_ALLPARTITIONS.gpt /dev/"$ssdVar" >/dev/null 2>&1
dd if=$saveDIR/SSD/CONF2/NASTAVNICKA/00_SSD_ALLPARTITIONS.gpt bs=512 count=1 > $saveDIR/SSD/CONF2/NASTAVNICKA/01_SSD_protectiveMBR.gpt
dd if=$saveDIR/SSD/CONF2/NASTAVNICKA/00_SSD_ALLPARTITIONS.gpt bs=512 skip=1 count=1 > $saveDIR/SSD/CONF2/NASTAVNICKA/02_SSD_primaryHEADER.gpt
dd if=$saveDIR/SSD/CONF2/NASTAVNICKA/00_SSD_ALLPARTITIONS.gpt bs=512 skip=2 count=1 > $saveDIR/SSD/CONF2/NASTAVNICKA/03_SSD_backupHEADER.gpt
dd if=$saveDIR/SSD/CONF2/NASTAVNICKA/00_SSD_ALLPARTITIONS.gpt bs=512 skip=3 > $saveDIR/SSD/CONF2/NASTAVNICKA/04_SSD_GPTPartitions.gpt

sgdisk --backup=$saveDIR/HDD/CONF2/NASTAVNICKA/00_HDD_ALLPARTITIONS.gpt /dev/"$hddVar" >/dev/null 2>&1
dd if=$saveDIR/HDD/CONF2/NASTAVNICKA/00_HDD_ALLPARTITIONS.gpt bs=512 count=1 > $saveDIR/HDD/CONF2/NASTAVNICKA/01_HDD_protectiveMBR.gpt
dd if=$saveDIR/HDD/CONF2/NASTAVNICKA/00_HDD_ALLPARTITIONS.gpt bs=512 skip=1 count=1 > $saveDIR/HDD/CONF2/NASTAVNICKA/02_HDD_primaryHEADER.gpt
dd if=$saveDIR/HDD/CONF2/NASTAVNICKA/00_HDD_ALLPARTITIONS.gpt bs=512 skip=2 count=1 > $saveDIR/HDD/CONF2/NASTAVNICKA/03_HDD_backupHEADER.gpt
dd if=$saveDIR/HDD/CONF2/NASTAVNICKA/00_HDD_ALLPARTITIONS.gpt bs=512 skip=3 > $saveDIR/HDD/CONF2/NASTAVNICKA/04_HDD_GPTPartitions.gpt